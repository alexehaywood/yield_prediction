    ...
    def rank_mols(mols, molecule_keys, rxn_component, yields):
        ...
        for mol in mols:
            ranked_mols.append({
                ...
                'mean_yield': np.mean(
                    round(yields.loc[yields[rxn_component] == mol].yield_exp, 3)
                    )
                })
...
def main():
    ...
    reactions_org = pd.read_excel(
        './data/original/reactions/rxns_all_no_nan_yields.xlsx'
    ...
    info['one-hot']['X_validation_train'], \
        info['one-hot']['X_validation_test'] = assemble_one_hot_encondings(
            rxn_components, 
            reactions_org.drop('yield_exp', axis='columns'), 
            reactions_val
            )
    info['one-hot']['X_validation_train'] = reactions_org.merge(
        info['one-hot']['X_validation_train'].reset_index(), 
        on=['additive', 'aryl_halide', 'base', 'ligand']
        ).set_index(
            ['additive', 'aryl_halide', 'base', 'ligand', 'yield_exp']
            )
    ...
    # for rxn_component, v in molecule_keys.items():
    #         molecule_keys[rxn_component] = v.to_dict()
    ...
    for d, info_d in info.items():
       ...
       #  # In-sample test.
       # test_name = 'in_sample'
       # in_sample_results = ml_tests.in_sample(X, y, descriptor_name, dirs)
        ...
        X_train = info_d['X_validation_train']
        y_train = info_d['X_validation_train'].reset_index('yield_exp').yield_exp
        X_test = info_d['X_validation_test']
