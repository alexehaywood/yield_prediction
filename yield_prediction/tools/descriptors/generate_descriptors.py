...
import tools.data_manipulation.rdkit_tools as rd
...
class descriptor_generator():
    ...
    def combine_descriptors_to_reactions(self, dict_df, rxns):
        """
        Combine descriptors to reactions.
        ...
        dict_df : dict of DataFrame
            The keys are the reaction components and the values are the
            DataFrames are the descriptors for each molecule.
        rxns : DataFrame
            Each row is a reaction. Each column is a specific reaction
            component.
        ...
        for k, df in dict_df.items():
            ...
            # for mol in df.name:
            for mol in rxns[k].drop_duplicates().to_list():
                if pd.isnull(mol):
                ...
                # Copy the descriptors row so the dataframe has the same length
                # as org.
                des_rep = pd.concat(
                    [des]*len(org),
                    ignore_index=True
                ...
                # Add the repeated descriptors dataframe to the original
                # reactions.
                ...
                # Add the reactions and descriptors to a dataframe containing
                # all molecules.
    ...
    def assemble_quantum_descriptors(
            self, rxn_components, reactions, dir_descriptors, saveas=None
            ):
        """
        Assemble quantum chemical descriptors.

        For the combinatorial reactions from the quantum descriptors of each
        reaction component.

        Parameters
        ----------
        rxn_components : list
            The names of the reaction components that also correspond to the
            names of the csv files that contain the quantum descriptors.
            E.g. 'aryl_halide', 'base' etc.
        reactions : DataFrame
            Molecules present in each reaction. Each row is a reaction. Each
            column is a specific reaction component.
        dir_descriptors : str
        ...
        quantum_descriptors = self.combine_descriptors_to_reactions(
    ...
    def get_smiles_data(
            self, rxn_components, reactions, hand_coded_smi, saveas=None
            ):
        """
        ...
        reactions : DataFrame
            Molecules present in each reaction. Each row is a reaction. Each
            column is a specific reaction component.
        hand_coded_smi : DataFrame
            Index is the molecule name and the column is the SMILES string.
        saveas : str, default=None
            Path and file name to save reaction SMILES to. The default is to
            not save.
                ...
                if pd.isnull(mol):
                    ...
                    if pd.isnull(smi):
                        try:
                            ...
                            print('{} is not in hand-coded smiles, please \
                                  update hand_coded_smi.xlsx'.format(mol))
        ...
        rxn_smiles = self.combine_descriptors_to_reactions(smiles, reactions)
    ...
    def assemble_graph_descriptors(
            self, rxn_components, reactions, rxn_smiles
            ):
        """
        Assemble molecular graphs.
        ...
        graph_descriptors = self.combine_descriptors_to_reactions(
            graphs_data, reactions
            )

        return graph_descriptors

    def assemble_fingerprint_descriptors(
            self, rxn_components, reactions, rxn_smiles,
            fp_type=rd.RDKFingerprint, fps_kw={}, return_raw=False,
            return_concat=False, return_sum=False
            ):

        if fp_type == 'rdk':
            fp_type = rd.RDKFingerprint
        elif fp_type == 'morgan':
            fp_type = rd.GetMorganFingerprintAsBitVect
        elif fp_type == 'maccs':
            fp_type = rd.GenMACCSKeys
        ...
        fp_descriptors_raw = self.combine_descriptors_to_reactions(
            fps_data, reactions
            )
        ...
        for ind, row in fp_descriptors_raw.iterrows():
            ...
            for i in row:
                if pd.isnull(i):
                    fps.append(
                        np.array([0 for i in np.arange(0, bit_size, 1)])
                        )
                else:
    ...
    # def assemble_one_hot_encondings(
    #         self, rxn_components, reactions, reactions_test=None
    #         ):
    ...
    #                 index_diff
    #                 )
    #         index_diff = pd.MultiIndex.from_frame(
    #             reactions_test
    #             ).names.difference(one_hot_encodings.index.names)
    #         if index_diff:
    ...
    #         return one_hot_encodings_reactions, \
    #             one_hot_encodings_reactions_test
    def assemble_one_hot_encondings(self, reactions):
        return reactions
